...
class Game extends SquareAABBCollidable {
    ...
    remove_snake_piece(removed:number):boolean
    {
        const view = new Int32Array(this.screen_buf.imageData!.data.buffer);
    ...
    init(width:number, height:number, cell_width:number, cell_height:number):void
    {
        ...
        const pixels = (new Array<RGB>(cell_height * cell_width)).fill(color, 0, cell_height * cell_width);
        //console.log(pixels)
        this.screen_buf = new Sprite(pixels,
    ...
    update_map():void
    {
        ...
        const color = new RGB(0, 0, 0, 255);
        this.heat_map = this.heat_map.map((cost:number, index:number, array:Int32Array) => {
